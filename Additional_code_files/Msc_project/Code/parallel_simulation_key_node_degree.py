...
def process_data(degree, in_degree, out_degree, target_id, sim_features, net_sim, curr_time):
...
def run_simulation_on_chunk(degree,
    ...
    for j in uninfected_followers_indices:
        ...
        target_index = j
        processed_df = process_data(degree, in_degree, out_degree, target_index, sim_features, net_sim, curr_time)
        valid_row = processed_df.iloc[[target_index]]
        valid = valid_row.drop(['user_id','infected_status','infection_time','followers_list',
                                'Nw_inDegree', 'generation', 'time_lapsed', 'seed_index', 'Nw_outDegree'],axis=1)

        valid = valid.astype('float64')

        columns = list(valid.columns)
        valid_value = valid[columns]

        pre_data = xgb.DMatrix(valid_value)
        score = model.predict(pre_data)
        if score > 0.6:
...
def main():
    ...
    #seeds = pd.read_csv(path+'mid_31-40_seeds_avg_neighbour_degree.csv')
    users['degree'] = users.apply(lambda x: len(x['target_nodes']), axis=1)
    users1 = users.sort_values('degree', ascending=False)
    seeds = users1[31:40]
    #seeds = seeds.loc[0:9, :]
    #seeds = seeds_5
    ...
    network_simulation.to_csv(path+'simulation_result_key_node_degree_mid_31-40.csv')
